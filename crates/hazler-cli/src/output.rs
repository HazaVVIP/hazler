...
impl OutputFormatter {
    ...
    pub fn format_tree(&self, result: &CrawlResult) -> String {
        ...
        for page in &result.pages {
            by_depth.entry(page.depth).or_insert_with(Vec::new).push(page);
        }
        ...
        for depth in 0..=max_depth {
            if let Some(pages) = by_depth.get(&depth) {
                for page in pages {
                    ...
                    let status_indicator = if page.status_code == 200 { "✓" } else { "✗" };
...
pub fn generate_stats(result: &CrawlResult) -> String {
    ...
    for page in &result.pages {
        let ct = page.content_type.clone().unwrap_or_else(|| "unknown".to_string());
...
pub fn generate_report(result: &CrawlResult) -> String {
    ...
    // Check for 404s
    let not_found: Vec<_> = result.pages.iter()
    ...
    // Check for 500s
    let server_errors: Vec<_> = result.pages.iter()
    ...
    // Check for redirects
    let redirects: Vec<_> = result.pages.iter()
